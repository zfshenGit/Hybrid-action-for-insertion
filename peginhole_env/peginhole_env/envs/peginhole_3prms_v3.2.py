...
class Peginhole_env(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def admittanceControl(self, BFxd, BFyd, BFzd, BTxd, BTyd, BTzd):
        ...
        BT = np.array([[BTxd, 0., 0.],
                       ...
                       [0., 0., BTzd]])  
        # F_d
        cur_Force_Target = np.array([[0, 0, 1]]).transpose() 
        cur_Torque_Target = np.array([[0, 0, 0]]).transpose()  
    ...
    def step(self,action):
        ...
        fullaction[0:3] = fullaction[0:3]*100
        ...
        action = np.concatenate((target_XYZ_send, target_RPY_send))
